...
export default class CollabyClient {
	...
	constructor(ip, connectCb, options) {
		...
		this.connect(ip, () => {
	      ...
	      this.sync();
			...
			// If Client is synced with server
			this.emitter.once('synced', (data) => {
	...
	connect(ip, cb) {
		console.log('connecting to http://' + ip + ":" + atom.config.get("collaby.port"));

		this.socket = io_cl('http://' + ip + ":" + atom.config.get("collaby.port"));
